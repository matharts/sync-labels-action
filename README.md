...
省略 `repositories` 或 `include` 表示同步全部合格仓库。显式 `include: []` 会被拒绝，避免配置错误把同步范围意外扩大到整个组织。默认全部模式会跳过 archived、disabled 和 fork 仓库；显式 Allowlist 遇到这些仓库时会停止并要求你更新策略。

### 4. 添加预览 workflow

创建 `.github/workflows/preview-labels.yml`。以下示例使用支持默认全部仓库和结构化 outputs 的 `v1.2.0`：
...
jobs:
  preview:
    ...
    steps:
      ...
      - uses: matharts/sync-labels-action@v1.2.0
        with:
...
正式使用时，把 `v1.2.0` 替换为该版本经过审核的完整提交 SHA。完整 SHA 不会随版本标签移动。
...
默认全部模式会跳过归档、禁用和 fork 仓库；显式 Allowlist 会拒绝这些仓库。单个仓库同步失败时，Action 会继续处理其他仓库，并在最后返回失败状态。如果写入过程中途失败，已经完成的操作不会回滚，job summary 和 outputs 会保留这些部分变更的计数。
...
- `repositories.include` 可省略；配置后必须非空，且仓库名称不能重复
...
| `dry_run` | 否 | `true` | 只预览变更；仅显式合法的假值会启用写入，其他值直接报错 |
...
`dry_run` 接受 `true/false`、`1/0`、`yes/no` 和 `on/off`（忽略大小写与首尾空格）；其他值会在访问 GitHub API 前失败。

## 使用 Action outputs

给 Action 步骤设置 `id` 后，可以直接在后续步骤中判断是否存在标签漂移，无需解析日志或 job summary：
...
所有 output 在 GitHub Actions 表达式中都是字符串。计数是本次运行处理的全部仓库的合计；dry-run 模式报告计划变更，写入模式报告成功完成的变更。
...
MathArts 的策略省略 `repositories.include`，因此预览和生产同步都会覆盖令牌可见的全部合格组织仓库。
