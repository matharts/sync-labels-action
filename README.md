...
| 模式 | 仓库访问 | 权限 |
| --- | --- | --- |
| 预览，`dry_run: "true"` | 策略中列出的全部仓库 | Issues: read |
| 写入，`dry_run: "false"` | 策略中列出的全部仓库 | Issues: write |
...
创建 `.github/label-policy.yml`。策略决定哪些标签归组织管理，以及哪些仓库参与同步：
...
这份策略允许 Action 管理所有 `type:*` 标签和 `help wanted`，并同步 `example` 与 `docs` 两个仓库。`bug` 是迁移中的旧名称，因此也属于受管标签。

### 4. 添加预览 workflow

创建 `.github/workflows/preview-labels.yml`。以下示例使用 `v1.0.2`，方便你先完成预览：
...
jobs:
  preview:
    ...
    steps:
      ...
      - uses: matharts/sync-labels-action@v1.0.2
        with:
...
正式使用时，把 `v1.0.2` 替换为经过审核的完整提交 SHA，例如 `fd09517b5a80583d41b21e85d70a145ad1be879f`。完整 SHA 不会随版本标签移动。
...
Action 会拒绝归档、禁用或 fork 仓库。单个仓库同步失败时，Action 会继续处理其他仓库，并在最后返回失败状态。
...
- `repositories.include` 必须明确列出至少一个仓库

同步使用显式 Allowlist。新仓库不会自动加入同步范围。
...
| `repository` | 否 | 空 | 只处理 Allowlist 中的一个仓库，可填写仓库名或 `owner/repository` |
...
- **`401` 或 `403`**: 确认令牌可以访问每个 Allowlist 仓库，并具有当前模式需要的 Issues 权限
