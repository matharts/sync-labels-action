...
这份策略允许 Action 管理所有 `type:*` 标签、`help wanted` 和旧名称 `bug`。其他标签属于仓库，Action 会保留它们。
...
Action 先为每个仓库生成并校验完整计划，再预览或执行该计划。dry-run 和真实写入使用相同的计划逻辑。
...
- [`SyncPlanner`](src/sync_planner.rb) 根据现有标签和配置生成计划
- [`SyncPlan`](src/sync_plan.rb) 校验并冻结计划
- [`SyncExecutor`](src/sync_executor.rb) 预览或执行计划
...
持续集成会在 Ruby 3.1、3.3 和 3.4 上运行测试，并使用 Actionlint 检查 workflow。
